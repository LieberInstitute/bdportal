...
export function loadData(allData) {
  ...
  for (let i=1;i<=numDataTypes;i++) {
      ...
      const j=i-1;
      dsoc[i]=[i]
      ld[j].forEach( (it)=>{ // [idx, name, public status (0/1), dbid, sample_count]
        dsoc[i].push(it[4]); //store sample count for this xtype
        ds[j].push(it[1]);   //   0    1        2                 3     4
...
export function anyActiveFilters() {
    for (const key in dtFilters) {
        ...
        } else if (key.startsWith('with_')) { //condition filter - only makes sense for Brain Matrix!
...
export function updateCounts() {
  ...
  ["dx", "age", "sex", "race"].forEach( (f)=>{
    let n=dtaNames[f].length;
    [ dtCounts[f], dtBrCounts[f], dtXCounts[f] ].forEach( (dt) => {
  ...
  const dtBrCountsRace=dtBrCounts.race;
  const dtBrCountsSex=dtBrCounts.sex;
  ...
  for (let xjj=0; xjj<xtiList.length; xjj++) {
    ...
    const dtBrCountsReg=dtBrCounts.reg[xt+1];
    ...
    console.log(`checking xt ${xt} for region filter:`, dtFilters.reg, dtaNames.reg[rrix])
    for (let i=0;i<len;++i) { //for each sample data row
        ...
        if (haveBrSet && !brSet.has(brix))  //hard skip of samples from this brain, not on the user given set!
                  continue;
        ...
        let newXBr = !dtBrAllsel.has(brix)
        //for region counts:
        ...
        //if (newXBrReg) { //new br-reg combo across all data
        //  dtBrXCountsReg[rg]++; // dtXCounts.reg[0]
        //  XBrRegs[brreg]=1;
        //  dtBrCountsXReg[rg]++;
        //}
    ...
    console.log(` ------------ S[${xt}] counts:`, dtXsel[xt].length, dtBrAllsel.size,
