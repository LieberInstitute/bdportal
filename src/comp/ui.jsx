...
const tooltipOpts={ delay: { show: 800, hide: 100 },
                   placement:'auto', trigger: 'hover', container: 'body'  }
const ttSel='[data-toggle="tooltip"]'
//should be called on first render useEffect( , [])

export function setupTooltips(eldom, opts) {
  if (eldom && $(eldom).length==0) return
  const jqs=eldom ? $(eldom).find(ttSel).addBack(ttSel)
                    : $(ttSel)
  if (!jqs.length) return
  if (opts) opts= { ...tooltipOpts, ...opts }
      else opts=tooltipOpts
  jqs.tooltip(opts)
  jqs.on('click.hint', (e) => {
          const t=$(e.target)
          t.tooltip('hide')
