...
let nodemw_srv=process.env.NODE_ENV=='development' ? 'http://localhost:4095' :
                      'http://gdebsrv:4095';
if (host.match(/^linwks/) || host.match(/^srv/)) {
  //dev at LIBD:
   ...
   //if
   nodemw_srv=(process.env.NODE_ENV=='development')? 'http://localhost:4095' :
      'http://192.168.77.16:4095'
}
process.env.VITE_NODESRV=nodemw_srv;
//console.log("~~~~~~~ vite.config:   NODE_ENV=", process.env.NODE_ENV)
//console.log("~~~~~~~ vite.config:   meta.env=", import.meta.env)
export default defineConfig({
  ...
  server: {
    ...
    proxy: { //only during development
      ...
      */
     '^/(pgdb|ruthere|pgplrinit)' : nodemw_srv
    }
