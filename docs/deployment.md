...
The intranet domain is `https://db.libd.net` (same server as `srv16.lieber.local`;
its TLS certificate is maintained). Use it for testing and links. The dev build
at `https://db.libd.net/dev/bdportal/` is the candidate for the next `/bdportal` release.

| Route (host `db.libd.net` / `srv16.lieber.local`) | Static files | Middleware |
| --- | --- | --- |
...
with `DEV_PORT=4097`, `NODE_ENV=production` and
`AUTH_URL=http://127.0.0.1:4017/` (node_auth over loopback, see below), as
user `gpertea`, group `www-data`. `server/.env` provides `DB_SRV`, `DB_USER`, `DB_PASS`, `JWTSHH`.
...
## Login path and the expired 6443 certificate

The production frontend still logs in through the old iframe
(`https://dev.libd.org/if_rlogin`), so it never calls the middleware `/auth`.
The devel frontend uses the same-origin dialog: browser -> `/…/api/auth` ->
middleware -> auth service. The middleware default `AUTH_SRV`
(`https://192.168.77.16:6443`) is an nginx TLS wrapper (`wg-auth-6443`) around
`node_auth.service` on `localhost:4017`; its `srv-v77-16` certificate expired
2026-04-28 and Node rejects it (`CERT_HAS_EXPIRED`). Since the middleware runs
on the same host, set `AUTH_URL=http://127.0.0.1:4017/` (full endpoint; nginx
mapped `/auth` to `/` on node_auth) to bypass the wrapper. The dev unit does
this; do the same when promoting the middleware to production. node_auth
answers 403 for a bad password, which the middleware reports as 401
`AUTH_FAILED`; the `useracc` check runs first and yields 403 `NO_PORTAL_ACCESS`.
